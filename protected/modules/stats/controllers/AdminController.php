...
class AdminController extends \humhub\modules\admin\components\Controller
{
    ...
    public function actionUserStats(){
        ...
        ->select(['u.*, count(e.id) as evidence_count, count(v.id) as review_count'])
        ->from('user as u')
        ->join('LEFT JOIN', 'evidence as e', 'e.created_by = `u`.`id`')
        ->join('LEFT JOIN', 'votes as v', 'v.user_id = `u`.`id`')
        ->groupBy('u.id')
        ->orderBy('u.id')
        ->all();
