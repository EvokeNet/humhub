 ...
 */
class Team extends ContentContainerActiveRecord implements \humhub\modules\search\interfaces\Searchable
{
    ...
    // TEAMS
    const TEAM_DISABLED = 0;
    const TEAM_ENABLED = 1;
    ...
    {
        return [
            [['join_policy', 'visibility', 'status', 'created_by', 'updated_by', 'auto_add_new_members', 'default_content_visibility','is_team'], 'integer'],
            [['name'], 'unique', 'targetClass' => self::className()],
            [['name'], 'required'],
            [['description', 'tags', 'color'], 'string'],
            [['created_at', 'updated_at'], 'safe'],
            [['is_team'], 'in', 'range' => [0, 1]],
    ...
    public function scenarios()
    {
        ...
        $scenarios['create'] = ['name', 'color', 'description', 'is_team', 'join_policy', 'visibility'];
    ...
    public function isMemberOfATeam($user_id){
        if(Team::getUserTeam($user_id) > 0 ){
            return true;
        }
